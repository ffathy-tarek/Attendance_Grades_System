...
const Attendance = () => {
  ...
  // دالة لتعديل حضور الطالب يدويًا
            ...
            <div key={course.id} className={styles.courseCard}>
              ...
              <div className={styles.courseBody}>
                ...
                <button 
                  ...
                  onClick={() => navigate(`/instructor/attendance/${course.id}`)}
                >
                  📝 Manage Attendance
                </button>
              </div>
  ...
  return (
    ...
    >
      ...
      {lectureSessions.length === 0 && (
        <div className={styles.card} style={{ marginBottom: "24px", background: "#fef3c7" }}>
      ...
      {lectureSessions.length > 0 && !selectedLecture && (
        <div className={styles.card} style={{ marginBottom: "24px" }}>
          ...
          <div className={styles.grid} style={{ gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", marginTop: "16px" }}>
            {lectureSessions.map(lecture => (
              <button
                ...
                style={{ padding: "12px" }}
              >
                {lecture.startTime?.toDate() ? new Date(lecture.startTime.toDate()).toLocaleTimeString() : 'Lecture'}
              </button>
            ))}
          </div>
        </div>
      ...
      {selectedLecture && (
        <div className={styles.card} style={{ marginBottom: "24px" }}>
          ...
          <p style={{ color: "#64748b", marginBottom: "16px" }}>
            Lecture: {selectedLecture.startTime?.toDate() ? new Date(selectedLecture.startTime.toDate()).toLocaleString() : 'Active'}
          </p>
      ...
      <div className={styles.summaryCards} style={{ marginBottom: "24px" }}>
        ...
        <div className={styles.summaryCard}>
          ...
          <div>
            ...
            <span className={styles.summaryValue} style={{ color: "#10b981" }}>{attendees.length}</span>
          </div>
        </div>
        <div className={styles.summaryCard}>
      ...
      {/* Students List with Manual Edit Buttons */}
      <div className={styles.tableWrapper}>
        <table className={styles.table}>
          ...
          <tbody>
              ...
              return (
                <tr key={student.id}>
                  ...
                  {attendanceSession && (
                    <td>
                      <button
                        onClick={() => toggleStudentAttendance(student.id, isPresent)}
                        style={{
                          padding: '6px 16px',
                          borderRadius: '8px',
                          fontSize: '13px',
            ...
            {students.length === 0 && (
              <tr>
                <td colSpan={attendanceSession ? "4" : "3"} style={{ textAlign: "center", padding: "40px", color: "#64748b" }}>
      ...
      {attendanceSession && (
        <div style={{ 
          ...
          <p>• Click on <strong>"Mark Present"</strong> to add a student to attendance manually.</p>
          <p>• Click on <strong>"Mark Absent"</strong> to remove a student from attendance manually.</p>
          <p>• Students can still register attendance through the mobile app during the active session.</p>
